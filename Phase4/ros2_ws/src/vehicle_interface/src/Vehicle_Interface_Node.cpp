...
{
    command_sub_ = this->create_subscription<vehicle_interface::msg::VehicleCommand>(
            "/vehicle_command",
            10,
            std::bind(
            &Car::command_callback,
            this,
            std::placeholders::_1));

    feedback_pub_ = this->create_publisher<vehicle_interface::msg::VehicleFeedback>(
            "/vehicle_feedback",
            10);
}
...
void Car::command_callback(const vehicle_interface::msg::VehicleCommand::SharedPtr msg){
    this->SetSteering(msg->steering);
    this->SetThrottle(msg->throttle);
    this->SetMode(msg->mode);

    RCLCPP_INFO(
        this->get_logger(),
        "Command: steering=%d throttle=%d mode=%u",
        msg->steering,
        msg->throttle,
        msg->mode);
}

// Start Car and TX/RX thread
...
// Keep the TxLoop running to send command to STM32
void Car::TxLoop()
{
...
//         }
// }
// Keep the RxLoop running to receive feedback from STM32 and publish to ROS2 feedback topic
void Car::RxLoop()
{
    while (running_.load()) {
        Feedback feedback{};
        ...
        vehicle_interface::msg::VehicleFeedback msg;

        msg.telemetry_seq = feedback.telemetry_seq;
        msg.last_cmd_seq = feedback.last_cmd_seq;
        msg.stm32_timestamp_ms = feedback.stm32_timestamp_ms;

        msg.erpm = feedback.erpm;
        msg.motor_current_x10 = feedback.motor_current_x10;
        msg.duty_x1000 = feedback.duty_x1000;
        
        // Send feedback to ROS2 topic
